int vp9_get_compressed_data ( VP9_COMP * cpi , unsigned int * frame_flags , size_t * size , uint8_t * dest , int64_t * time_stamp , int64_t * time_end , int flush ) {
 ...
 # if CONFIG_SPATIAL_SVC vp9_svc_lookahead_peek ( cpi , cpi -> lookahead , 0 , 1 ) ;
 # endif vp9_restore_layer_context ( cpi ) ;
 }
 ...
 # if CONFIG_SPATIAL_SVC if ( is_two_pass_svc ( cpi ) ) source = vp9_svc_lookahead_peek ( cpi , cpi -> lookahead , arf_src_index , 0 ) ;
 ...
 # if CONFIG_SPATIAL_SVC if ( is_two_pass_svc ( cpi ) ) last_source = vp9_svc_lookahead_peek ( cpi , cpi -> lookahead , - 1 , 0 ) ;
