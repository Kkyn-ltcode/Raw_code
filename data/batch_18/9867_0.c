static void __attribute ( ( noinline ) ) print_statistics ( hp_timing_t * rtld_total_timep ) {
 # ifndef HP_TIMING_NONAVAIL char buf [ 200 ] ;
 char * cp ;
 char * wp ;
 ...
 * wp = '\0' ;
 ...
 # endif unsigned long int num_relative_relocations = 0 ;
 for ( Lmid_t ns = 0 ;
 ns < GL ( dl_nns ) ;
 ++ ns ) {
 ...
 # ifndef ELF_MACHINE_REL_RELATIVE if ( ( l -> l_addr != 0 || ! l -> l_info [ VALIDX ( DT_GNU_PRELINKED ) ] ) && l -> l_info [ VERSYMIDX ( DT_RELACOUNT ) ] ) # else if ( l -> l_addr != 0 && l -> l_info [ VERSYMIDX ( DT_RELACOUNT ) ] ) # endif num_relative_relocations += l -> l_info [ VERSYMIDX ( DT_RELACOUNT ) ] -> d_un . d_val ;
 }
 }
 ...
 # ifndef HP_TIMING_NONAVAIL if ( HP_SMALL_TIMING_AVAIL ) {
 char pbuf [ 30 ] ;
